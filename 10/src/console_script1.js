...
function read_async() {
  console.log(
    `5.\t\x1B[32mWykonano pierwszą linię funkcji "read_async()"\x1B[0m`
  );
  console.log("6.\t\x1B[33mWywołano funkcję 'readFile()'\x1B[0m");
  fs.readFile(argv[1], (err, data) => {
    if (err) throw err;
    console.log(
      "7.\t\x1B[33mWczytano zawartość pliku — jest ona dostępna w zmiennej 'data'\x1B[0m"
    );
  });
  console.log(
    `8.\t\x1B[32mWykonano ostatnią linię funkcji "read_async()"\x1B[0m`
  );
}
/************************* */
